...

{
  ...
  home.file =
    ...
    in
    {
      ...
      # Solo los mounts rclone via XDG autostart (Plasma/Cinnamon): sin conflicto con HM services
      ...
      # Antigravity (VSCode extension config)
      ".config/antigravity".source = link "Antigravity/.config/antigravity";
  ...
  home.activation.cinnamonDconf = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    if command -v cinnamon &>/dev/null && [ -f "$HOME/dotfiles-dizzi/cinnamon/.config/cinnamon/settings.dconf" ]; then
      dconf load /org/cinnamon/ < "$HOME/dotfiles-dizzi/cinnamon/.config/cinnamon/settings.dconf" 2>/dev/null || true
    fi
  '';
