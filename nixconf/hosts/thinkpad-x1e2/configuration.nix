...

{
  ...
  # (evita "Freezing user space processes failed" por I/O en FUSE) y los vuelve a
  # montar como el usuario tras despertar.
  environment.etc."systemd/system-sleep/90-rclone.sh".source = pkgs.writeShellScript "90-rclone-sleep.sh" ''
    ...
    case "$1/$2" in
      ...
      post/*)
        $sleep_bin 8
        $runuser_bin -u diego -- $env_bin HOME=/home/diego \
          $rclone_bin mount gdrive:/ /home/diego/mi_gdrive --vfs-cache-mode full &
        $runuser_bin -u diego -- $env_bin HOME=/home/diego \
          $rclone_bin mount gd-musica:/ /home/diego/mi_gdmusica --vfs-cache-mode full &
        $runuser_bin -u diego -- $env_bin HOME=/home/diego \
          $rclone_bin mount gd-libros:/ /home/diego/mi_gdlibros --vfs-cache-mode full &
        ;;
    esac
    exit 0
  '';
